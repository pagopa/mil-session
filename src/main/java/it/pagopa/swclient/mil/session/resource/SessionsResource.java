package it.pagopa.swclient.mil.session.resource;
...
import it.pagopa.swclient.mil.session.bean.CreateSessionResponse;
import it.pagopa.swclient.mil.session.bean.GetSessionResponse;
import it.pagopa.swclient.mil.session.bean.Outcome;
import it.pagopa.swclient.mil.session.bean.TaxCodeSource;
import it.pagopa.swclient.mil.session.bean.UpdateSessionRequest;
import it.pagopa.swclient.mil.session.bean.UpdateSessionResponse;
...
import it.pagopa.swclient.mil.session.dao.Pairing;
import it.pagopa.swclient.mil.session.dao.PairingService;
...
public class SessionsResource {
		         ...
		         .chain(ctxTaxCode -> {
		            if (ctxTaxCode.get() != null) {
		               ...
		               return pmWalletService.retrieveTaxCode(createSessionRequest.getPanToken())
		                     ...
		                     .map(res ->  {
		                    	 Log.debugf("RetrieveTaxCodeResponse %s", res.toString());
	...
	@Produces(MediaType.APPLICATION_JSON)
	@Path("/")
	public Uni<Response> getSessionByPairingToken(@Valid @BeanParam CommonHeader commonHeader,
									@NotNull(message = "[" + ErrorCode.PAIRING_TOKEN_MUST_NOT_BE_NULL + "] pairingToken must not be null")
									@Pattern(regexp = "^\\d{6}$", message = "[" + ErrorCode.PAIRING_TOKEN_MUST_MATCH_REGEXP + "] pairingToken must match \"{regexp}\"")
									@RestQuery(value = "pairingToken") String pairingToken) {
		Log.debugf("getSessionByPairingToken - Input parameters: %s pairingToken [%s]", commonHeader, pairingToken);

		
		Uni<Pairing> pairing = pairingService.getdel(pairingToken)
				.onFailure().transform(t -> {
					Log.errorf(t, "[%s] REDIS error while deleting session", ErrorCode.REDIS_ERROR_WHILE_DELETING_SESSION);
					return new InternalServerErrorException(Response
						.status(Status.INTERNAL_SERVER_ERROR)
		...
		return Uni.createFrom()
		        .item(pairing.map(Pairing::getTaxCode))
		        .attachContext()
		        .chain(ctxTaxCode -> Uni.createFrom().item(ctxTaxCode))
		        .chain(ctxTaxCode -> {
		        	CreateSessionRequest createSessionRequest = new CreateSessionRequest();
		        	createSessionRequest.setTaxCode(ctxTaxCode.context().get(CONTEXT_TAX_CODE));
		        	// use the tax code to retrieve the tc acceptance and save new card flags
		            return createSessionWithTaxCode(ctxTaxCode.context().get(CONTEXT_TAX_CODE), commonHeader, createSessionRequest);
		        });
